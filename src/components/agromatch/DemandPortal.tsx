...
const DemandPortal = ({ demands, supplies, cart, setCart, onAddToCart, onCreateOrder, onAddDemand }: DemandPortalProps) => {
  ...
  const banks = [
    { id: 'BSI', name: 'Bank Syariah Indonesia', logo: '🏦' },
            ...
            <div className="bg-teal-600 p-6 text-white flex justify-between items-center">
              <div className="flex items-center space-x-3">
                {checkoutStep === 'payment' && (
                  <button onClick={() => setCheckoutStep('info')} className="hover:bg-white/20 p-2 rounded-full transition mr-2">
                    <ArrowLeft size={20} />
                  </button>
                )}
                ...
                <h3 className="font-bold text-lg">
                  {checkoutStep === 'info' ? 'Checkout Keranjang Belanja' : 'Pilih Metode Pembayaran'}
                </h3>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-8">
              {checkoutStep === 'info' ? (
                ...
                <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                  <div className="space-y-4">
                    <h5 className="text-sm font-bold text-slate-900 flex items-center">
                      <CreditCard size={16} className="mr-2 text-teal-600" /> Pilih Metode Pembayaran
                    </h5>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      >
                        <div className={cn(
                          ...
                          paymentMethod === 'Transfer Bank' ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-400"
                        )}>
                          <CreditCard size={24} />
                        </div>
                        <div>
                      ...
                      >
                        <div className={cn(
                          "w-12 h-12 rounded-2xl flex items-center justify-center",
                          paymentMethod === 'COD' ? "bg-teal-600 text-white" : "bg-slate-100 text-slate-400"
                        )}>
                          <Wallet size={24} />
                        </div>
                        <div>
                  ...
                  {paymentMethod === 'Transfer Bank' && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                      <h5 className="text-sm font-bold text-slate-900 flex items-center">
                        <Store size={16} className="mr-2 text-teal-600" /> Pilih Bank Transfer
                      </h5>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {banks.map((bank) => (
                          ...
                          >
                            ...
                            <h6 className="text-[10px] font-bold text-slate-900 uppercase">{bank.name}</h6>
                          </div>
                        ))}
                      </div>
                      <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex items-start gap-3">
                        ...
                        <p className="text-[11px] text-amber-800 leading-relaxed">
                          Pesanan akan diproses setelah pembayaran diverifikasi. Pastikan nominal transfer sesuai hingga 3 digit terakhir.
                        </p>
                      </div>
                    </div>
                  )}
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              {checkoutStep === 'info' ? (
                ...
                <Button 
                  ...
                  className="w-full sm:w-auto bg-teal-600 hover:bg-teal-700 text-white font-bold px-10 py-6 rounded-2xl shadow-lg transition-all"
                >
